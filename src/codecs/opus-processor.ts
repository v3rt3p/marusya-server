...
import { OggParser } from './ogg-parser'
...
export function parseOggFile (data: Buffer): {
  data: Buffer
  head: OpusHead,
} {
  ...
  let encoder: OpusEncoder | undefined
  const oggPages = OggParser.parse(data)
  for (const page of oggPages) {
    if (page.pageSequenceNumber === 0) {
      head = parseOpusHead(Buffer.concat(page.segments))
      encoder = new OpusEncoder(head.sampleRate, head.channelCount)
      continue
    }
    ...
    for (const segment of page.segments) {
      const decoded = encoder.decode(segment)
      buffers.push(decoded)
    }
  }

  if (!head) {
    throw new Error('No OpusHead parsed')
  }
